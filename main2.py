...
# Accessing Cameras
camera_out = cv2.VideoCapture(2)
camera_in = cv2.VideoCapture(1)
...
# Persistent state to track if any face has been detected in "camera in"
face_detected_in_camera_in = False
...
while True:
    ...
    for face_encoding in face_encodings_out:
        ...
        if matches[best_match_index]:
            ...
            thickness = 3
            lineType = 2
            cv2.putText(frame_out, name + " Present", bottom_left_corner_of_text, font, fontScale, fontColor, thickness,
                        lineType)
    ...
    for face_encoding in face_encodings_in:
        ...
        if matches[best_match_index]:
            ...
            thickness = 3
            lineType = 2
            cv2.putText(frame_in, name + " Present", bottom_left_corner_of_text, font, fontScale, fontColor, thickness,
                        lineType)

            detected_faces_in[name] = True
            face_detected_in_camera_in = True  # Set the persistent state

    # Check if both Jai and Anil have been detected at least once in "camera out"
    if all(detected_faces_out.values()) and not face_detected_in_camera_in:
        ledPin.write(1)
    # Check if a face has been detected at least once in "camera in"
    if face_detected_in_camera_in:
        ledPin.write(0)
